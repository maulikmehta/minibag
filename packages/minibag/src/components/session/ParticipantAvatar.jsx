...
function ParticipantAvatar({
  ...
  return (
    ...
    >
      {/* First name popup */}
      {showPopup && firstName && (
        <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 z-[60] px-3 py-1.5 bg-gray-900 text-white text-sm rounded-lg shadow-lg whitespace-nowrap">
          {firstName}
          {/* Arrow pointing up */}
      ...
      <div className={`${sizeClasses} rounded-full flex items-center justify-center mb-2 transition-all ${borderClass} bg-white relative ${hasItems ? 'animate-bounce-in' : ''}`}>
        ...
        {/* Active selection checkmark badge - bottom-left */}
        {isSelected && (
          <div
            className="absolute -bottom-0.5 -left-0.5 w-5 h-5 bg-green-600 rounded-full flex items-center justify-center border-2 border-white shadow-sm"

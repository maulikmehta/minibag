 ...
 * @param sessionId - Session UUID (not short sessionId)
 * @returns Rate limit check result
 */
export async function checkPinAttempt(sessionId: string): Promise<RateLimitResult> {
  const now = new Date();
  ...
  if (attempt.attempts >= CONFIG.MAX_TOTAL_ATTEMPTS) {
    ...
    console.warn('[PinRateLimiter] Session blocked:', {
      sessionId,
 ...
 * @param attemptedPin - The PIN that was attempted (for logging)
 */
export async function recordFailedPinAttempt(sessionId: string, attemptedPin?: string | null): Promise<void> {
  const now = new Date();
  const nowMs = now.getTime();
  ...
  if (!existing) {
    ...
    console.info('[PinRateLimiter] First failed attempt:', { sessionId, attempts: 1 });
  } else {
    // Increment attempts
    ...
    console.warn('[PinRateLimiter] Failed attempt recorded:', {
      sessionId,
      attempts: newAttempts,
 ...
 * @param sessionId - Session UUID
 */
export async function clearPinAttempts(sessionId: string): Promise<void> {
  try {
    await prisma.pinAttempt.delete({
      where: { sessionId }
    });

    console.info('[PinRateLimiter] Attempts cleared after successful auth:', { sessionId });
  } catch (error: any) {
    // Record may not exist (first attempt was successful) - this is OK
    if (error.code !== 'P2025') {
      console.error('[PinRateLimiter] Failed to clear attempts:', { error, sessionId });
    }
  }
}
...
async function cleanupExpiredEntries(): Promise<void> {
  ...
  try {
    ...
    if (result.count > 0) {
      console.debug('[PinRateLimiter] Cleanup:', { cleanedCount: result.count });
    }
  } catch (error) {
    console.error('[PinRateLimiter] Cleanup failed:', error);
  }
}
 ...
 */
export function startPinRateLimiterCleanup(): () => void {
  console.info('[PinRateLimiter] Starting cleanup job...');
  ...
  return () => {
    ...
    console.info('[PinRateLimiter] Stopped cleanup job');
  };
}
 ...
 */
export async function getPinAttemptCount(sessionId: string): Promise<number> {
  const attempt = await prisma.pinAttempt.findUnique({
    where: { sessionId },
